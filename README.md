...
- **Ограниченные ресурсы**: Сервера с 1-2 GB памяти требует оптимизации кода, снижения количества I/O операций и эффективного управления памятью.
- **Анализ данных**: Данные должны сниматься синхронно и по случайным таймингам, что требует наличие координатора и системы работы с ошибками/задержками.
- **Настройка ClickHouse для работы с сервером 2ГБ RAM**: Clickhouse не предназначен для работы на небольших инстансах, поэтому требует тонкой настройки для работоспособности


#### 🔧 Реализация
Всего участвуют пять серверов
1. На сервере координаторе работают:
   - Скрипт Python который анализирует готовность скраперов, генерирует случайное время для одновременного парсинга коэффицентов, управляет событиями и перезапуском инстансов в случае ошибок
   - Система обмена данными(out - тайминги, in - данные) через Redis. Выбран за легкость, простоту и скорость. Потери данных маловероятны и не страшны
   - Airflow - контроль работы, перезапуск скриптов, очередность запуска
   - Producer Kafka - для загрузки данных на сервер с БД. Kafka исключительно в целях демонстрации и обучения, есть возможность загружать данные гапрямую в Clickhouse без шины 
   - Скрипт Python с анализом и обработкой поступаемых данных - сопоставление резльтатов, подбор лучших коэффицентов
   - Дэшборд Grafana для визуализации из Clickhouse
2. Три сервера собирают данные с букмекерских сайтов. В определенные промежутки времени, заданные координатором, считываются LIVE ставки, так же считываются ставки прематч и результаты
3. Сервер БД:
   - Receiver Kafka
   - Clickhouse - выбор обусловлен отсутсвием изменений данных, и большим количеством аналитических онлайн запросов для дэшборда
